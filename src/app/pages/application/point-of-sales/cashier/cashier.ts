...
export class Cashier implements OnInit, OnDestroy {
    ...
    private loadActiveShift() {
        // Subscribe to active shift from state (persists across navigation)
        this._store.select(POSShiftState.getActiveShift)
            .pipe(takeUntil(this.Destroy$))
            .subscribe(shift => {
                this.ActiveShift = shift;
                // If there's an active shift, also restore the cashier
                if (shift && shift.cashier_id) {
                    const cashier = this.Employees.find(e => e.id === shift.cashier_id);
                    if (cashier) {
    ...
    printReceipt() {
        ...
        printReceiptHelper(
            ...
            this.SelectedCashier?.full_name || "-",
            this.Setting
        );
    }
